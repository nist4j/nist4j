...
class RT2UserDefinedDescriptionTextRecordImmutableImplTest {

  @Test
  void getIFieldTypeEnumValues() {
    // Given
    NistRecordBuilder builder =
        new RT2UserDefinedDescriptionTextNistRecordBuilderImpl(
            OptionsFixtures.OPTIONS_DONT_CHANGE_ON_BUILD);
    // When
    NistRecord nistRecord = new RT2UserDefinedDescriptionTextRecordImmutableImpl(builder);

    // Then
    assertThat(nistRecord).isNotNull();
    assertThat(nistRecord.getIFieldTypeEnumValues()).hasSize(2);
  }
}
