...
class RT1TransactionInformationRecordImmutableImplTest {

  @Test
  void getIFieldTypeEnumValues() {
    // Given
    NistRecordBuilder builder =
        new RT1TransactionInformationNistRecordBuilderImpl(
            OptionsFixtures.OPTIONS_DONT_CHANGE_ON_BUILD);

    // When
    NistRecord nistRecord = new RT1TransactionInformationRecordImmutableImpl(builder);

    // Then
    assertThat(nistRecord).isNotNull();
    assertThat(nistRecord.getIFieldTypeEnumValues().size()).isEqualTo(18);
  }
}
