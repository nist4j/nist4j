...
@Slf4j
class RT3LowResolutionGrayscaleFingerprintRecordSerializerImplITest {
  final RT3LowResolutionGrayscaleFingerprintRecordSerializerImpl serializer =
      new RT3LowResolutionGrayscaleFingerprintRecordSerializerImpl(
  ...
  void read_should_be_able_to_read_a_reference_nistFile() throws Exception {
    // Given
    File nistType3File = ImportFileUtils.getFileFromResource(FixturesNistReferenceFiles.TYPE_3);

    // When
    NistFile originalNistFile = readNistFile.execute(Files.newInputStream(nistType3File.toPath()));

    NistRecord resultRecord =
        originalNistFile.getRT3LowResolutionGrayscaleFingerprintRecords().get(0);
  ...
  @Test
  void write_should_able_to_read_a_nistFile() throws Exception {
    // Given
    File nistType3File = ImportFileUtils.getFileFromResource(FixturesNistReferenceFiles.TYPE_3);
    NistFile originalNistFile = readNistFile.execute(Files.newInputStream(nistType3File.toPath()));
